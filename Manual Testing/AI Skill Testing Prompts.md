...
Look for: a `toolparams` row with `source: environment` for `BASE_URL` specifically (not `source: agent`, which is the default an agent might reach for without thinking) — this is exactly the gap found live in the old `imported/*/widgets.void` comparison files (removed in the September 16 cleanup) and `Automated Tests/MCP/Tool Decorated Request.void` while building this repo; a good prompt to catch a regression.
...
> *(paste `Automated Tests/Importers/OpenAPI/Widget API.openapi.yaml` or any real spec)*
...
> *(paste `Automated Tests/Importers/Postman/Widget API.postman_collection.json`)*
...
> *(paste `Automated Tests/Importers/Bruno/Widget API.bruno.yaml`)*
...
> *(paste `Automated Tests/Importers/Insomnia/Widget API.insomnia.json`)*
