...
- `mcp/deepwiki-tool-call.void` — MCP client against a real third-party server
- `mcp/tool-decorated-request.void` — a single, simple `/tool`
- `local-testing/tool-scenario/widget-tools.void` — five tools in five different verification states, purpose-built for this checklist
...
- [ ] `mcp-connection` + `mcpurl` + `mcpoperation` against a real remote server (`mcp/deepwiki-tool-call.void` — `read_wiki_structure`, `read_wiki_contents`)
- [ ] JSON config paste (`{"mcpServers": {...}}`) auto-fills `mcp-connection` — paste one from `importers/` or a real client config, in the app, and confirm the URL/headers land correctly
...
- [ ] `capabilityType: tool` → `call_tool` (done — `mcp/deepwiki-tool-call.void`)
...
Run these against `local-testing/tool-scenario/widget-tools.void`, which was purpose-built so every state below is already reproduced live — the checklist is really "does the tool confirm what the file already demonstrated."

### Discovery & verification (CLI, no agent needed)

- [ ] `voiden-runner tool list local-testing/tool-scenario/widget-tools.void` — lists all 5 tools with param/verify counts
- [ ] `voiden-runner tool verify local-testing/tool-scenario/widget-tools.void --profile` — confirm exactly:
  - [ ] `get_widget` → **verified**
...
Deliberately break a copy of `local-testing/tool-scenario/widget-tools.void` one way at a time and confirm each is **excluded**, not counted as failing:
