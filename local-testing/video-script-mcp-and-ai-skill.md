...
That's the actual demo. It's not contrived — it's the real workflow this repo's own `importers/` → `imported/` → `local-testing/tool-scenario/` pipeline walked through while building it, just narrated.
...
**On screen:** talking-head or voiceover-over-blank-editor. Maybe `importers/postman/widget-api.postman_collection.json` open, unscrolled.
...
> "Convert `importers/postman/widget-api.postman_collection.json` into Voiden requests."
...
**Cut to the generated output** (or just open `imported/from-postman/widgets.void`, already built, if the live generation takes too long to fully show):
...
**Beat — run it:**
```
voiden-runner run imported/from-postman/widgets.void --profile --no-session
```
...
**On screen:** `crud-and-chaining/users-crud.void` or `rest/headers-query-path.void` — pick whichever reads cleanest on screen.
...
**This is the core of the video.** On screen: `local-testing/tool-scenario/widget-tools.void`.
...
**On screen:** `mcp/deepwiki-tool-call.void`.
...
```
voiden-runner run mcp/deepwiki-tool-call.void --profile --no-session
```
